...
import { operationLogConfig } from "src/config/operationLog";
...
export const ormConfigs = {
  host: operationLogConfig.db.host,
  port: operationLogConfig.db.port,
  user: operationLogConfig.db.user,
  dbName: config.DB_NAME ?? operationLogConfig.db.dbName,
  password: config.DB_PASSWORD ?? operationLogConfig.db.password,
  type: "mysql",
  forceUndefined: true,
  runMigrations: true,
  migrations: {
  ...
  entities,
  debug: operationLogConfig.db.debug,
  seeder: {
