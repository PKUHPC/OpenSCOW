...
export const USER_TOKEN_COOKIE_KEY = "SCOW_USER";
const COOKIE_PATH = "/";
...
export function getTokenFromCookie(ctx?: ParseCookieContext): string | undefined {
  return parseCookies(ctx)[USER_TOKEN_COOKIE_KEY] ?? undefined;
}
...
export function setTokenCookie(ctx: SetCookieContext, token: string) {
  setCookieWithAge(ctx, USER_TOKEN_COOKIE_KEY, token);
}

export function destroyUserInfoCookie(ctx: SetCookieContext) {
  destroyCookie(ctx, USER_TOKEN_COOKIE_KEY, { path: COOKIE_PATH });
}
