...
export const config = envConfig({
  ...
  LDAP_URL: str({ desc: "LDAP地址。认证类型为ldap必填" }),
  LDAP_SEARCH_BASE: str({ desc: "LDAP用户搜索base。认证类型为ldap必填" }),
  LDAP_BIND_DN: str({ desc: "操作LDAP时以什么用户操作，默认为空字符串", default: "" }),
  LDAP_BIND_PASSWORD: str({ desc: "操作LDAP的用户的密码，默认为空字符串", default: "" }),
  LDAP_FILTER: str({ desc: "LDAP用户筛选器。认证类型为ldap必填" }),
  LDAP_ADD_USER_BASE: str({ desc: "LDAP增加用户节点时，把用户增加到哪个节点下。认证类型为LDAP必填。" }),
  LDAP_ADD_GROUP_BASE: str({ desc: "LDAP增加用户对应的组时，把组节点增加到哪个节点下。认证类型为LDAP必填。" }),
