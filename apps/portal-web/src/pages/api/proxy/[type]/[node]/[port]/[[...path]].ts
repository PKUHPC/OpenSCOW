 ...
 * 所以整个系统第一次启动后，在以HTTP形式访问此函数之前，到代理地址的WebSocket将会失败
 * 因为此时并没有注册upgrade事件的监听器，无法处理WebSocket连接。
 *
 * 所以系统启动后，需要手动触发一次到本地址的HTTP请求，以便注册upgrade事件的监听器
  */
export function registerWebSocketProxyServer(res: AugmentedNextApiResponse) {
  if (res.socket.server.upgrade) { return false; }
  // @ts-ignore
