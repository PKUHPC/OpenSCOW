...
import { NoticeType } from "@scow/notification-protos/build/common_pb";
...
export default route(GetUnreadMessageSchema, async (req, res) => {
  ...
  return notifClient.message.getUnreadMessages({ userId: info.identityId, noticeType: NoticeType.SITE_MESSAGE })
    .then((res) => {
      return { 200: { results: {
