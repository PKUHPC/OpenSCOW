...
import { CreateAccountHandlersName, CreateAccountStatus } from "src/asyncOperations/handlers/createAccount";
...
import { UserAccount, UserRole as EntityUserRole } from "src/entities/UserAccount";
import { getAccountStateInfo } from "src/utils/accountUserState";
import { toRef } from "src/utils/orm";

export const asyncAccountServiceServer = plugin((server) => {
  ...
  server.addService<AccountServiceServer>(AccountServiceService, {
    ...
    createAccount: async ({ request, em, logger }) => {
      ...
      if (account) {
        throw <ServiceError> {
          code: Status.ALREADY_EXISTS, message: `Account ${accountName} is already exists.`,
        };
      }

      await asyncOperationRequests?.createLongRunningOperation({
        type: CreateAccountHandlersName,
        status: CreateAccountStatus.CREATING_CLUSTERS_ACCOUNT,
        params: { accountName, tenantName, ownerId, comment },
        progress: {},
      }, logger);
