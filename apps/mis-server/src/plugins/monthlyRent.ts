...
export interface MonthlyRentPlugin {
  fetch: {
    started: () => boolean;
    start: () => void;
    stop: () => void;
...
export const MonthlyRentPlugin = plugin(async (f) => {

  let fetchStarted = !!misConfig.monthlyRent;
  ...
  f.addExtension("fetch", <MonthlyRentPlugin["fetch"]>{
