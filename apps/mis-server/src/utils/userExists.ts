...
export async function userExists(userId: string, logger: Logger, em: SqlEntityManager<MySqlDriver>) {
  const capabilities = await getCapabilities(config.AUTH_URL || misConfig.authUrl);
  ...
  const userInfo = await getUser(config.AUTH_URL || misConfig.authUrl, { identityId: userId }, logger);
  return {
    existsInScow: !!user,
