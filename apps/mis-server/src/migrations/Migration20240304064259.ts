...
export class Migration20240304064259 extends Migration {
  ...
  async up(): Promise<void> {
    ...
    this.addSql("update `account` a set a.state = CASE WHEN a.whitelist_id IS NOT NULL AND a.blocked_in_cluster = 0 THEN \'NORMAL\' WHEN a.whitelist_id IS NULL AND a.blocked_in_cluster = 1 AND a.balance > COALESCE(a.block_threshold_amount, (SELECT t.default_account_block_threshold FROM `tenant` t WHERE a.tenant_id = t.id)) THEN \'BLOCKED_BY_ADMIN\' ELSE \'NORMAL\' END;");
  }
