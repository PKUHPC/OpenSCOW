...
费用项目的信息存放在数据库的`job_price_item`表中。每次需要计算价格时，服务器将会从数据库中取得所有计费项，并得到每个(集群,分区,QOS)的最新的计费规则。如果在[集群信息配置文件](../../../deploy/config/clusterConfig.md)的某个QOS不存在费用项，服务器将会报错。
